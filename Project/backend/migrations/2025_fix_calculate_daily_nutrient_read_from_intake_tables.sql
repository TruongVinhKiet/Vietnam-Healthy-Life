-- Migration: Fix calculate_daily_nutrient_intake to read fiber and fatty acids from UserFiberIntake and UserFattyAcidIntake
-- These tables are populated by triggers, so we should read from them instead of recalculating from FoodNutrient
...
BEGIN
    ...
    -- FIXED: Read from UserFiberIntake (populated by trigger) instead of recalculating from FoodNutrient
    fiber_intake AS (
        SELECT 
    ...
    -- FIXED: Read from UserFattyAcidIntake (populated by trigger) instead of recalculating from FoodNutrient
