...
BEGIN
    IF NEW.created_by_user IS NOT NULL THEN
        INSERT INTO drinknotification (
            ...
            FORMAT('Your drink "%s" has been created successfully! It is awaiting admin approval.', COALESCE(NEW.vietnamese_name, NEW.name))
        );
    END IF;
...
BEGIN
    IF NEW.is_public = TRUE AND OLD.is_public = FALSE AND NEW.created_by_user IS NOT NULL THEN
        INSERT INTO drinknotification (
            ...
            'drink_approved',
            'Drink approved',
            FORMAT('Congratulations! Your drink "%s" has been approved and is now public.', COALESCE(NEW.vietnamese_name, NEW.name))
        );
    END IF;
