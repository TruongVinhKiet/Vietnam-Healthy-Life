...
BEGIN
  ...
  -- Backfill FoodNutrient for food_id=110 (ÄÃ¡ láº¡nh)
  ...
  -- Backfill FoodNutrient for food_id=50 (Rau muá»‘ng)
  ...
  IF EXISTS (SELECT 1 FROM dish WHERE dish_id = 106) THEN
    ...
    FROM food f
    WHERE (f.name ILIKE '%egg%' OR COALESCE(f.name_vi, '') ILIKE '%trá»©ng%')
    ORDER BY EXISTS (SELECT 1 FROM foodnutrient fn WHERE fn.food_id = f.food_id) DESC,
             f.food_id
    LIMIT 1;
