...
class CreateDishScreen extends StatefulWidget {
  ...
  @override
  createState() => _CreateDishScreenState();
}
...
class _CreateDishScreenState extends State<CreateDishScreen>
  ...
  static const Map<String, Map<String, String>> _dishCategoryMeta = {
    'main_course': {'label': 'Món Chính', 'icon': '🍛'},
    'soup': {'label': 'Súp/Canh', 'icon': '🍲'},
    'salad': {'label': 'Salad', 'icon': '🥗'},
    'side_dish': {'label': 'Món Phụ', 'icon': '🍽️'},
    'appetizer': {'label': 'Khai Vị', 'icon': '🥗'},
    'breakfast': {'label': 'Bữa Sáng', 'icon': '🍳'},
    'dessert': {'label': 'Tráng Miệng', 'icon': '🍰'},
    'snack': {'label': 'Đồ Ăn Vặt', 'icon': '🍿'},
  };

  List<Map<String, String>> _categories = [
    for (final entry in _dishCategoryMeta.entries)
      {
        'value': entry.key,
  ...
  Future<void> _checkNameExists() async {
    ...
    try {
      final exists = await DishService.checkNameExists(
  ...
  Future<void> _loadDishCategories() async {
    try {
      final rows = await DishService.getCategories();
      ...
      for (final row in rows) {
        final value = (row['category'] ?? '').toString();
        if (value.isEmpty || seen.contains(value)) continue;
        ordered.add(value);
        seen.add(value);
      }

      for (final key in _dishCategoryMeta.keys) {
      ...
      final mapped = ordered.map((value) {
        final meta = _dishCategoryMeta[value];
      ...
      debugPrint('Error loading dish categories: $e');
    }
  }
  ...
  Future<void> _searchFoods(String query) async {
    if (query.isEmpty) {
      // Reload initial food list when search is cleared
      _loadInitialFoods();
      return;
    }
  ...
  Future<void> _submitDish() async {
        ...
        SnackBar(
          content: Builder(
            builder: (context) {
              ...
              return Text(l10n.pleaseEnterDishName);
            },
          ),
    ...
    try {
      ...
      if (_selectedImageBytes != null) {
        ...
        try {
          imageUrl = await DishService.uploadDishImage(
            _selectedImageBytes!,
      ...
      await DishService.createDish(
        ...
        vietnameseName: _vietnameseNameController.text.trim().isNotEmpty
        ...
        servingSizeG: totalWeight,
        imageUrl: imageUrl,
        ingredients: _ingredients
            .map(
              (ing) => {'food_id': ing['food_id'], 'weight_g': ing['weight_g']},
            )
            .toList(),
      );
      ...
      if (mounted) {
        ...
        final dishName = _vietnameseNameController.text.trim().isNotEmpty
            ? _vietnameseNameController.text.trim()
            : _nameController.text.trim();
        await LocalNotificationService().notifyDishCreated(dishName);
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Builder(
              builder: (context) {
                final l10n = AppLocalizations.of(context)!;
                return Text(l10n.dishCreatedSuccessfully);
              },
            ),
        ...
        children: [
          ...
          TextField(
            ...
            decoration: InputDecoration(
              labelText: 'Tên món ăn (tiếng Anh) *',
              labelStyle: const TextStyle(
                fontFamily: FitnessAppTheme.fontName,
                fontSize: 14,
              ),
              hintText: 'vd: Grilled Chicken',
              filled: true,
          ...
          TextField(
            ...
            decoration: InputDecoration(
              labelText: 'Tên món ăn (tiếng Việt)',
              labelStyle: const TextStyle(
                fontFamily: FitnessAppTheme.fontName,
                fontSize: 14,
              ),
              hintText: 'vd: Gà Nướng',
              filled: true,
              fillColor: FitnessAppTheme.background,
              border: OutlineInputBorder(
                borderRadius: BorderRadius.circular(12),
          ...
          TextField(
            ...
            decoration: InputDecoration(
              ...
              hintText: 'vd: Món gà nướng thơm ngon, ít dầu mỡ...',
              filled: true,
              fillColor: FitnessAppTheme.background,
              border: OutlineInputBorder(
                borderRadius: BorderRadius.circular(12),
      ...
      child: Row(
        children: [
          Container(
            ...
            child: const Icon(Icons.food_bank, color: Colors.green, size: 20),
          ),
          const SizedBox(width: 12),
          Expanded(
            child: Column(
        ...
        return Transform.scale(
          ...
          child: Container(
            ...
            child: FloatingActionButton.extended(
              ...
              label: Text(
                _isSubmitting ? 'Đang tạo...' : 'Tạo Món Ăn',
                style: const TextStyle(
                  fontFamily: FitnessAppTheme.fontName,
                  fontWeight: FontWeight.w700,
                  fontSize: 17,
