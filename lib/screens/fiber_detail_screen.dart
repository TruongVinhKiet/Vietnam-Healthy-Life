...
class FiberDetailScreen extends StatefulWidget {
  final int fiberId;
  final String title;
  final String? code;
  
  const FiberDetailScreen({
    super.key,
  ...
  @override
  State<FiberDetailScreen> createState() => _FiberDetailScreenState();
}

class _FiberDetailScreenState extends State<FiberDetailScreen> {
  ...
  Future<void> _load() async {
    ...
    // Load fiber details
    final res = await AuthService.getFiberById(widget.fiberId);
    
    // Load nutrient tracking to get consumption percentage
    // Note: Fiber is not yet in the tracking function, so this will return 0%
    ...
    if (code != null) {
      for (final nutrient in nutrients) {
        if (nutrient['nutrient_code'] == code && 
            (nutrient['nutrient_type']?.toString().toLowerCase() ?? '') == 'fiber') {
  ...
  Color _getFiberColor(String? code) {
    if (code == null) return const Color(0xFF8A98E8);
    
    // Fiber color mapping (from fiber_view.dart)
    const colorMap = {
      'SOLUBLE': 0xFF98D8C8,
      'INSOLUBLE': 0xFFD4A373,
    };
  ...
  Widget build(BuildContext context) {
    ...
    final fiberName = v['name']?.toString() ?? widget.title;
    final fiberCode = v['code']?.toString() ?? '';
    final color = _getFiberColor(fiberCode);
            ...
            child: Column(
              ...
              children: [
                ...
                _buildSection(
                  ...
                  Column(
                    children: [
                      if (fiberCode.isNotEmpty)
                        _buildInfoRow('Mã', fiberCode),
                      _buildInfoRow('Nhóm', 'Dietary Fiber'),
                      if (v['unit'] != null)
