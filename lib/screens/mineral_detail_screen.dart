...
class _MineralDetailScreenState extends State<MineralDetailScreen> {
  ...
  Future<void> _load() async {
    ...
    final res = await AuthService.getMineralById(widget.mineralId);
    
    // Load nutrient tracking to get consumption percentage
    final nutrients = await AuthService.getDailyNutrientTracking();
    double percentage = 0.0;
    
    // Get the mineral code (either from widget parameter or from API response)
    final code = widget.code ?? (res != null && res['code'] != null ? 'MIN_${res['code']}' : null);
    
    if (code != null) {
      for (final nutrient in nutrients) {
        if (nutrient['nutrient_code'] == code && 
            (nutrient['nutrient_type']?.toString().toLowerCase() ?? '') == 'mineral') {
  ...
  Color _getMineralColor(String? code) {
    if (code == null) return const Color(0xFF8A98E8);
    
    // Remove MIN_ prefix if present
    final cleanCode = code.replaceAll('MIN_', '');
    ...
    return Color(colorMap[cleanCode] ?? 0xFF8A98E8);
  }
  ...
  Widget build(BuildContext context) {
    ...
    final mineralName = v['name']?.toString() ?? widget.title;
    final mineralCode = v['code']?.toString() ?? '';
    final color = _getMineralColor(mineralCode.isNotEmpty ? 'MIN_$mineralCode' : null);
            ...
            child: Column(
              ...
              children: [
                ...
                _buildSection(
                  ...
                  Column(
                    children: [
                      if (mineralCode.isNotEmpty)
                        _buildInfoRow('Mã', mineralCode),
                      _buildInfoRow('Nhóm', 'Mineral'),
                      if (v['unit'] != null)
