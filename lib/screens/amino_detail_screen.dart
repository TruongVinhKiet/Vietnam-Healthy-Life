...
class AminoDetailScreen extends StatefulWidget {
  final int aminoId;
  final String title;
  final String? code;
  
  const AminoDetailScreen({
    super.key,
  ...
  @override
  State<AminoDetailScreen> createState() => _AminoDetailScreenState();
}

class _AminoDetailScreenState extends State<AminoDetailScreen> {
  ...
  Future<void> _load() async {
    ...
    // Load amino acid details
    final res = await AminoService.getAminoById(widget.aminoId);
    
    // Load nutrient tracking to get consumption percentage
    // Note: Amino acids are not yet in the tracking function, so this will return 0%
    ...
    if (code != null) {
      for (final nutrient in nutrients) {
        if (nutrient['nutrient_code'] == code && 
            (nutrient['nutrient_type']?.toString().toLowerCase() ?? '') == 'amino_acid') {
  ...
  Color _getAminoColor(String? code) {
    if (code == null) return const Color(0xFF8A98E8);
    
    // Amino acid color mapping (from amino_view.dart)
    const colorMap = {
      'HIS': 0xFFFF6F61,
      'ILE': 0xFF6495ED,
      'LEU': 0xFF32CD32,
  ...
  Widget build(BuildContext context) {
    ...
    final aminoName = v['name']?.toString() ?? widget.title;
    final aminoCode = v['code']?.toString() ?? '';
    final color = _getAminoColor(aminoCode);
            ...
            child: Column(
              ...
              children: [
                ...
                _buildSection(
                  ...
                  Column(
                    children: [
                      if (aminoCode.isNotEmpty)
                        _buildInfoRow('Mã', aminoCode),
                      _buildInfoRow('Nhóm', 'Amino acid'),
                      if (v['home_display'] != null)
                        _buildInfoRow(
                          'Hiển thị',
                          v['home_display'] ? 'Yes' : 'No',
                        ),
                      ...
                      children: foods.map((food) {
                        return Card(
                          ...
                          child: ListTile(
                            ...
                            subtitle: food['amount_per_100g'] != null
                                ? Text(
                                    '${food['amount_per_100g']} / 100g',
                                    style: const TextStyle(
                                      fontFamily: FitnessAppTheme.fontName,
                                    ),
                                  )
